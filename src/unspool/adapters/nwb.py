"""Lossless, session-aware adapters between NWB trials and :class:`unspool.Study`."""
...
_EMBEDDED_SUBJECT = "unspool_subject"
_EMBEDDED_SESSION = "unspool_session"
_EMBEDDED_TRIAL = "unspool_trial"
_EMBEDDED_SESSION_ORDER = "unspool_session_order"
_EMBEDDED_COLUMNS = {
    _EMBEDDED_SUBJECT,
    _EMBEDDED_SESSION,
    _EMBEDDED_TRIAL,
    ...
    *,
    ...
    for name, values in export_columns.items():
        ...
        nwbfile.add_trial_column(
            name=name,
            description=descriptions.get(name, f"Unspool Study column {name!r}."),
        )
...
def _require_pynwb() -> tuple[Any, Any, Any]:
    try:
        ...
        raise RuntimeError(
            "NWB interoperability requires optional dependencies; install `unspool[nwb]`"
        ) from error
