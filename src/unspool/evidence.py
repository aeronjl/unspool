...
from unspool.protocol import ProtocolState, protocol_from_json
from unspool.reporting import ReportedProtocol

BUNDLE_SCHEMA_VERSION = "unspool.evidence-bundle/1"
_FORBIDDEN_SUFFIXES = {
    ".pkl",
    ".pickle",
    ".joblib",
...
def capture_environment() -> dict[str, Any]:
    ...
    for package in ("unspool", "numpy", "scipy"):
        try:
...
def replay_evidence_bundle(bundle_or_path: EvidenceBundle | str | Path) -> BundleReplay:
    ...
    elif evaluation.get("schema_version") == "unspool.nested-evaluation-report/1":
