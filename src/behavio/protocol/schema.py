...
#: :attr:`~behavio._internal.scoring.ComparisonMultiplicity.BENJAMINI_HOCHBERG`, which
...
class ObservationSpec:
    ...
    :func:`behavio.protocol.compiler.materialize_protocol`; they are declarations the cohort must
...
class ComparisonSpec:
    ...
    :mod:`behavio.protocol.runner` applied unconditionally before the member existed, so adopting
...
class StudyProtocol:
    ...
    def __post_init__(self) -> None:
        ...
        if self.schema_version not in ACCEPTED_PROTOCOL_SCHEMA_VERSIONS:
            raise ProtocolValidationError(
                f"unsupported schema_version {self.schema_version!r}; "
                f"expected one of {ACCEPTED_PROTOCOL_SCHEMA_VERSIONS!r}"
            )
        if (
            self.schema_version in PRE_MULTIPLICITY_PROTOCOL_SCHEMA_VERSIONS
    ...
    def amend(self, *, identifier: str, reason: str, **changes: Any) -> Self:
        ...
        The amended draft is stamped with the current :data:`PROTOCOL_SCHEMA_VERSION`. It is a new
...
def _comparison_from_payload(payload: Any, schema_version: Any) -> ComparisonSpec:
    ...
    if schema_version in PRE_MULTIPLICITY_PROTOCOL_SCHEMA_VERSIONS:
