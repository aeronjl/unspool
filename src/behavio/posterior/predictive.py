...
``PredictiveDiscrepancy`` and ``PredictiveTail`` are declared in
:mod:`behavio.contracts.discrepancy` -- that is where a third party writes a new
discrepancy -- and are re-exported here beside the check that consumes them.
...
:func:`behavio.posterior.diagnostics.audit_posterior` on the same result and retains the
...
:class:`~behavio.posterior.diagnostics.PosteriorAuditPolicy` whose per-check severities
...
from behavio._internal import scoring as _multiplicity
from behavio._internal.scoring import adjust_probabilities, excess_probability
...
from behavio.posterior.result import ArviZUnavailableError, PosteriorError, PosteriorResult
...
class PredictiveMultiplicity(StrEnum):
    ...
    :class:`~behavio.compare.models.ComparisonMultiplicity` through
    :mod:`behavio._internal.scoring`, which also owns the step-up itself, so the two
...
def posterior_predictive_check(
    ...
    *,
    ...
    :class:`~behavio.posterior.diagnostics.PosteriorAuditPolicy` that names the severities
