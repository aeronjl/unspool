...
- **posterior convergence**, delegated to :func:`behavio.posterior_diagnostics.audit_posterior`.

They combine by worst severity, following
:class:`~behavio.posterior_diagnostics.PosteriorAuditStatus`: any ``ERROR``-severity issue
...
ineligible in :mod:`behavio.runner`. The full :class:`~behavio.posterior_diagnostics.
...
class PSISLOOIssue:
    ...
    ``severity`` mirrors :class:`~behavio.posterior_diagnostics.PosteriorAuditIssue`: an
...
class PSISLOOResult:
    ...
    refusal is enforced in :func:`behavio.posterior_comparison.compare_posterior_models`.
    """
    ...
    *,
    ...
    Args:
        ...
        policy: Thresholds for the convergence audit. ``None`` uses the default
            :class:`~behavio.posterior_diagnostics.PosteriorAuditPolicy`.
