...
``ObjectiveTarget``, ``PriorMeasure`` and the ``OptimizationBackend`` protocol are declared
in :mod:`behavio.contracts.backend` -- an extension author implements them there -- and are
re-exported here beside the machinery that drives them. ``OptimizationProblem``,
``OptimizationAttempt`` and ``OptimizationRun`` are defined here: they carry substantial
validation logic and need a concrete
:class:`~behavio.inference.parameters.ParameterSpace`, which ``behavio.contracts`` must not
import at runtime.
"""
...
from behavio.inference.parameters import ParameterSpace, ParameterSpaceError
...
class PyBADSMultistart:
    ...
    derived unsigned 32-bit seed. That is what lets a seed emitted by
    :mod:`behavio.posterior.simulation_based_calibration`, :mod:`behavio.recovery.parameters`, or
    :mod:`behavio.recovery.models` be handed straight to this
