...
class HistoryTerm:
    ...
    Only :mod:`behavio.design.formula` sets it, because only there is ``coding="effect"`` chosen
...
class DesignSpec:
    ...
    def from_formula(cls, formula: str, *, training_study: Study | None = None) -> DesignSpec:
        ...
        ``formula`` uses the notation documented in :mod:`behavio.design.formula`, for example
    ...
    def describe(self) -> str:
        ...
        Raises :class:`~behavio.design.formula.FormulaError` if a third-party term has no formula
        spelling. Use :attr:`signature` when an unparseable fingerprint is enough.
        """

        from behavio.design.formula import describe_design

        return describe_design(self)

    def __str__(self) -> str:
        from behavio.design.formula import describe_term
