...
Both functions present what :class:`~behavio.sbc.SBCSummary` and
:class:`~behavio.sbc.SBCUniformity` already carry. Neither recomputes a rank, a band, or a
test statistic.
"""
...
from behavio.sbc import SBCSummary, SBCUniformity
...
def plot_sbc_rank_histogram(
    ...
    *,
    ...
    The reference line is :attr:`~behavio.sbc.SBCSummary.expected_bin_count`, the count each
    bin would hold under the exact discrete uniform null. No binomial envelope is drawn
    around it, because the calibrated reference for deviation is the *simultaneous* band on
    :func:`plot_sbc_ecdf_difference` and a second, pointwise envelope here would invite the
    eye to read bin-by-bin exceedances as significant.
...
def plot_sbc_ecdf_difference(
    ...
    *,
    ...
    :attr:`~behavio.sbc.SBCUniformity.confidence_level`. Error bars would suggest a
    pointwise reading, which would be exceeded far more often than the nominal level.

    Evaluation points where the curve leaves the band are marked individually and counted,
    matching :attr:`~behavio.sbc.SBCUniformity.n_points_outside_band`.
    """
