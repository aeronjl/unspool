...
:mod:`behavio.pose`, :mod:`behavio.ethograms` and :mod:`behavio.covariates` hold
behaviour as it was observed: float64 seconds on a named physical ``clock_id``.
:class:`behavio.study.Study` holds behaviour as it was designed: one row per
...
is reduced. This follows :class:`behavio.clocks.ClockSpec`, which makes a
longitudinal time coordinate an explicit object rather than a convention, and
:meth:`behavio.covariates.BehaviorCovariate.aligned_to`, which refuses a clock
mismatch instead of assuming one. Two clocks are related only by
:class:`behavio.sync.ClockSynchronization`; :meth:`TrialTiming.synchronized_to`
...
inside a training fold with :func:`behavio.transforms.fit_transform_split`.
...
from behavio.ethograms import BehaviorAnnotations
from behavio.study import REQUIRED_COLUMNS, Study
from behavio.sync import ClockSynchronization
...
class TrialTiming:
    ...
    numbering as the :class:`behavio.study.Study` rows the reduction will be
    ...
    def synchronized_to(
        ...
        *,
        ...
        accepted :class:`behavio.sync.ClockSynchronization`, retaining its
...
def reduce_covariate_to_trials(
    ...
    *,
    ...
    :meth:`behavio.covariates.BehaviorCovariate.aligned_to`: consecutive valid
...
def _require_fold_independent(reducer: Any) -> None:
    if not getattr(reducer, "fold_independent", False):
        raise TrializationError(
            ...
            "with behavio.transforms.fit_transform_split."
        )
...
def _require_matching_stream(
    ...
    if clock_id != timing.clock_id:
        raise TrializationError(
            ...
            f"{timing.clock_id!r}; relate them with fit_clock_synchronization() and "
