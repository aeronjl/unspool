"""What one trial of a behavioural task means, declared rather than inferred.

A :class:`TaskSpec` names the observed columns of a :class:`~behavio.trials.Study` and what
each of them *is*: which column holds the choice and what its alternatives are, whether a
reward was delivered, when the clock for a response time started. Nothing here fits
anything; the specification is what makes a fit interpretable.

This module types itself against :mod:`behavio.contracts.estimator` rather than against
:mod:`behavio.models.base`, so the task layer sits *below* the model catalogue: a model may
declare the task columns it requires without the task contract having to know that any
