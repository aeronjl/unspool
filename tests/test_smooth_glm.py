...
def smooth_model(*, n_sessions: int = 8, shared_trajectory: bool = False) -> SmoothModel:
    return smooth(
        base_glm(),
        over="session_order",
...
def smooth_truth(model: SmoothModel) -> MappingProxyType[str, float]:
...
def test_stationary_truth_does_not_require_a_smooth_advantage() -> None:
    ...
    smooth_loss = -np.mean(
        np.concatenate(
            [item.pointwise_log_probability for item in evaluate_splits(drifting, study, splits)]
        )
    )
