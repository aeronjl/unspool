...
import pytest
from test_bounded_report import recovered, report_items
from test_runner import candidate_models, compiled_nested

from behavio.contracts.fold import EvaluationFold
from behavio.posterior.diagnostics import PosteriorAudit, PosteriorAuditPolicy
from behavio.posterior.loo import PSISLOOResult
from behavio.protocol.runner import run_nested_protocol
...
#: report to ``behavio.evaluate.folds-report/2``. The eighteen archived files were diffed one
...
#: Recomputed a fifth time by the module reorganisation and the ``covariates`` ->
#: ``predictors`` rename. Nothing was computed differently; three *declared strings* moved.
#: ``TransformSpec.implementation`` in the shared fixture names
#: ``behavio.time.transforms.Standardize`` where it named ``behavio.transforms.Standardize``;
#: ``ValidationSpec.splitter`` names ``behavio.evaluate.splits.cohort_forward_session_splits``
#: where it named ``behavio.validation.cohort_forward_session_splits``; and each candidate's
#: declared hyperparameter is named ``predictors`` where it was named ``covariates``. A
#: protocol records the dotted path and the hyperparameter names of the code it declares, so
#: renaming that code is a change to the declaration and must change its fingerprint -- that
#: is the property the fingerprint exists to have. The bundle was diffed file by file:
#: ``protocol/protocol.json`` differs in exactly those strings, and every other identity moved
#: only because it chains off the protocol fingerprint. No estimate, score, interval, audit or
    ...
    with zipfile.ZipFile(original) as source, zipfile.ZipFile(forged, "w") as target:
        for info in source.infolist():
            ...
            if info.filename == "bundle.json":
                ...
                manifest["schema_version"] = "behavio.report.evidence_bundles-bundle/99"
...
def test_the_declared_fold_identifier_keys_the_prediction_and_audit_maps() -> None:
    ...
    assert all(isinstance(split, EvaluationFold) for split in (fold.split for fold in folds))
