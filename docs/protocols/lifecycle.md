...
The current version is `behavio.study-protocol/4`. Four superseded names are still read:

| Superseded name | What it lacks | How it is read |
| --- | --- | --- |
| `behavio.study-protocol/3` | `ComparisonSpec.metrics` | the table carries the one column its runner reported, `(metric,)` |
| `behavio.study-protocol/2` | that, and `CandidateSpec.inference` | every candidate is read as `optimized`, which is the only kind its runner could execute |
...
recorded against it. `to_dict` therefore omits `multiplicity` from a version 1 record,
`inference` from a version 1 or 2 record and `metrics` from anything before version 4, and
`StudyProtocol` refuses to construct such a protocol carrying any value other than the one
its era could express — so nothing distinguishable is ever dropped.
