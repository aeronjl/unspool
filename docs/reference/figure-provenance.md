...
Figures are part of Behavio's scientific argument, not decorative screenshots. This
...
generated by
[`scripts/plot_documentation_figures.py`](https://github.com/aeronjl/behavio/blob/main/scripts/plot_documentation_figures.py)
with fixed styling and deterministic SVG identifiers; the first-analysis display is
generated by its exact checked-in example. The two behaviour-boundary diagrams are hand-authored
SVGs committed alongside their pages; they moved here from `fipha` with the code they
describe and carry no generator script. Rebuild the generated set with:
...
| `behavior-ecosystem-v0.1.svg` | Show which observed-behaviour exchange type each upstream behaviour tool feeds, and where Behavio's responsibility starts |
| `interval-policy-pipeline-v0.1.svg` | Show the ordered bout operations and the ledger, fingerprint, and encoding inputs they produce |
