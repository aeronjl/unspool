...
landmark are not interchangeable with it. They should be stored as distinct columns.

Later releases will add typed clock and landmark operations. In particular, a landmark
estimated from behaviour must be fitted using training observations only before it is used
to transform a validation fold. The current contract deliberately does not guess or align
such landmarks.
