...
Unspool keeps probabilistic inference outputs behind a small, backend-neutral contract.
...
purpose is labelled, reproducible interoperability across inference backends. Unspool uses
...
A PyMC, HSSM, NumPyro, or Stan result can already be analysed with ArviZ. Unspool's added
...
from unspool import PosteriorGroup, PosteriorResult, PosteriorVariable
...
`chain` and `draw`; every sampled group must use the same coordinates. Unspool currently
...
uses the supported ArviZ 0.22-0.23 `InferenceData` representation. Unspool tests both paths
...
```python
from unspool import posterior_result_from_arviz
